...
def normalizeImage(img):
    ...
    #1.Set 'out'= 'img' subtracted by the minimum of the image.
    out = img - np.amin(img)
    #2. multiply 'out' by (255 / max) w/ max is the max value of 'out'.
...
def linearWeight(pixel_value):
    ...
    if (pixel_value > pixel_range_mid):
        weight = np.float64 (pixel_range_max - pixel_value)
    else:
        weight = np.float64 (pixel_value)
...
def computeResponseCurve(pixels, log_exposures, smoothing_lambda,
    ...
    for i in xrange(pix_range):
        for j in xrange(num_images):
            ...
            #1a. Set mat_A[idx_ctr, pixel_value] to the value of the weighting
            #function (wij). pixel_value is the value in pixels at i, j.
            mat_A[idx_ctr, pixels[i,j]] = wij

            #1b. Set mat_A [idx_ctr, pix_range + i] to the negative value of the
            #weighting function.
            mat_A[idx_ctr, pix_range + i] = -wij

            #1c. Set mat_b [idx_ctr, 0] to the value of the weighting function
            #multiplied by the log_exposure at j.
