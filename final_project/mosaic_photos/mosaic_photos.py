...
""" Final Project - Building an Mosaic image

This mosaic_photos will take the photos input and convert image to mosaic image

usage:
 python mosaic_photo <image> <tiles directory>
Note: tiles must be square!
...
OUT_FILE = 'mosaic_photo_final.jpg'
EOQ_VALUE = None

TILE_BLOCK_SIZE = PX_TILE_SIZE / max(min(TILE_MATCH_RES, PX_TILE_SIZE), 1)
...
class TileProcessor:
	...
	def __process_tile(self, tile_path):
		try:
			...
			large_tile_img = img.resize((PX_TILE_SIZE, PX_TILE_SIZE), Image.ANTIALIAS)
			small_tile_img = img.resize((PX_TILE_SIZE/TILE_BLOCK_SIZE, PX_TILE_SIZE/TILE_BLOCK_SIZE), Image.ANTIALIAS)
...
class TargetImage:
	...
	def get_data(self):
		...
		w = img.size[0] * IMG_ENLARGEMENT
		h = img.size[1]	* IMG_ENLARGEMENT
		large_img = img.resize((w, h), Image.ANTIALIAS)
		w_diff = (w % PX_TILE_SIZE)/2
		h_diff = (h % PX_TILE_SIZE)/2
		
		# if necesary, crop the image slightly 
		# so we use a whole number of tiles horizontally and vertically
		if w_diff or h_diff:
...
class TileFitter:
	...
	def __get_tile_diff(self, t1, t2, bail_out_value):
		...
		for i in range(len(t1)):
			...
			if diff > bail_out_value:
				# we know already that this isnt going to be 
				# the best fit, so no point continuing with this tile
				return diff
		return diff
	...
	def get_best_fit_tile(self, img_data):
		...
		# go through each tile in turn looking for the best match 
		# for the part of the image represented by 'img_data'
...
class MosaicImage:
	def __init__(self, original_img):
		...
		self.x_tile_count = original_img.size[0] / PX_TILE_SIZE
		self.y_tile_count = original_img.size[1] / PX_TILE_SIZE
		self.total_tiles  = self.x_tile_count * self.y_tile_count

	def add_tile(self, tile_data, coords):
		img = Image.new('RGB', (PX_TILE_SIZE, PX_TILE_SIZE))
	...
	try:
		...
		for x in range(mosaic.x_tile_count):
			for y in range(mosaic.y_tile_count):
				large_box = (x * PX_TILE_SIZE, y * PX_TILE_SIZE, (x + 1) * PX_TILE_SIZE, (y + 1) * PX_TILE_SIZE)
				small_box = (x * PX_TILE_SIZE/TILE_BLOCK_SIZE, y * PX_TILE_SIZE/TILE_BLOCK_SIZE, (x + 1) * PX_TILE_SIZE/TILE_BLOCK_SIZE, (y + 1) * PX_TILE_SIZE/TILE_BLOCK_SIZE)
		...
		# put these special values onto the queue 
		# to let the workers know they can terminate
		for n in range(WORKER_COUNT):
			work_queue.put((EOQ_VALUE, EOQ_VALUE))

def mosaic_photo(image_path, tiles_path):
	tiles_data = TileProcessor(tiles_path).get_tiles()
	image_data = TargetImage(image_path).get_data()
...
if __name__ == '__main__':
	if len(sys.argv) < 3:
		...
		mosaic_photo(sys.argv[1], sys.argv[2])
